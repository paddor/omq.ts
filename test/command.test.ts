...
describe("decodeCommand", () => {
  ...
  it("rejects truncated name", () => {
    expect(() => decodeCommand(new Uint8Array([10, 0x41]))).toThrow("truncated")
  })
...
describe("decodeReadyProperties edge cases", () => {
  ...
  it("handles multiple custom properties", () => {
    function prop(key: string, val: Uint8Array): Uint8Array {
      ...
      buf.set(keyBytes, 1)
      new DataView(buf.buffer).setUint32(1 + keyBytes.byteLength, val.byteLength, false)
  ...
  it("preserves unknown properties", () => {
    ...
    const extra = new Uint8Array(1 + customKey.byteLength + 4 + customVal.byteLength)
    extra[0] = customKey.byteLength
    extra.set(customKey, 1)
    new DataView(extra.buffer).setUint32(1 + customKey.byteLength, customVal.byteLength, false)
