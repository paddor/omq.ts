...
import { encodeReady, encodeSubscribe, encodeCancel, encodeJoin, encodeLeave, decodeCommand, decodeReadyProperties } from "../src/command.ts"
...
function makeReadyWithIdentity(ws: MockWebSocket, serverType: string, identity: string): void {
  ws.simulateOpen()
  const ready = encodeReady(serverType as "PUSH", new TextEncoder().encode(identity))
  ws.simulateMessage(encodeCommandFrame(ready))
}

function sendDataFrame(ws: MockWebSocket, ...parts: (string | Uint8Array)[]): void {
...
describe("Req", () => {
  it("sends with empty delimiter and receives reply", async () => {
    ...
    sendDataFrame(ws, new Uint8Array(0), new Uint8Array([0, 0, 0, 1]), "snapshot data")
  ...
  it("multipart reply preserves all parts after delimiter", async () => {
    ...
    sendDataFrame(ws, new Uint8Array(0), new Uint8Array([0, 0, 0, 42]), "part1", "part2")
...
describe("Rep", () => {
  ...
  it("preserves routing envelope from ROUTER peer", async () => {
    const rep = new Rep()
    rep.connect("ws://localhost:8082")
    const ws = createdSockets[0]!
    makeReady(ws, "ROUTER")

    // ROUTER sends: [identity, empty delimiter, request]
...
describe("Router", () => {
  ...
  it("routes send to correct peer by identity", async () => {
    ...
    expect(dataFramesAfterReady(ws1).length).toBe(1)
    expect(dataFramesAfterReady(ws2).length).toBe(0)
    expect(new TextDecoder().decode(dataFramesAfterReady(ws1)[0]![0]!)).toBe("for-A")
  })
  ...
  it("silently drops send to unknown identity", async () => {
    ...
    await router.send(new Message(new TextEncoder().encode("nonexistent"), "hello"))
...
describe("Pair", () => {
  ...
  it("rejects second connection", () => {
    const pair = new Pair()
    pair.connect("ws://localhost:8081")
    expect(() => pair.connect("ws://localhost:9081")).toThrow("only one connection")
  })
...
describe("Server", () => {
  ...
  it("routes reply by routing ID", async () => {
    ...
    await server.send(Message.fromParts([routingId, new TextEncoder().encode("reply-to-1")]))
    expect(dataFramesAfterReady(ws1).length).toBe(1)
    expect(dataFramesAfterReady(ws2).length).toBe(0)
  })
  ...
  it("assigns distinct routing IDs to different peers", async () => {
    ...
    const id1 = new DataView(msg1.parts[0]!.buffer, msg1.parts[0]!.byteOffset, 4).getUint32(0, false)
    const id2 = new DataView(msg2.parts[0]!.buffer, msg2.parts[0]!.byteOffset, 4).getUint32(0, false)
  ...
  it("silently drops send with invalid routing ID size", async () => {
    ...
    await server.send(Message.fromParts([new Uint8Array(3), new TextEncoder().encode("nope")]))
    expect(dataFramesAfterReady(ws).length).toBe(0)
  })
})
...
describe("Peer", () => {
  ...
  it("routes send by routing ID", async () => {
    ...
    await peer.send(Message.fromParts([routingId, new TextEncoder().encode("reply")]))
    expect(dataFramesAfterReady(ws1).length).toBe(1)
    expect(dataFramesAfterReady(ws2).length).toBe(0)
  })
})
...
describe("Channel", () => {
  ...
  it("rejects second connection", () => {
    const channel = new Channel()
    channel.connect("ws://localhost:8081")
    expect(() => channel.connect("ws://localhost:9081")).toThrow("only one connection")
  })
})
